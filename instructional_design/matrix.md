...
| :--: |-------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------|:----------:|----------------------------------------------|
| ex21 | Customer Cohort Revenue Growth                        | How does total revenue accumulate over time for each cohort of customers, grouped by first purchase month?                                 | `CTE`, window functions (`SUM OVER`, `ROWS BETWEEN`), `DATE_TRUNC`, joins     |     8      | **CUSTOMER → ORDERS → LINEITEM**             |
| ex22 | Top 3 Parts by Revenue in Each Region                 | Which parts generated the most revenue within each region — and how can we identify top performers efficiently?                            | Multi-level `JOIN`, `RANK() OVER (PARTITION BY)`, hierarchical joins, CTEs    |     8      | **REGION → NATION → SUPPLIER → PARTSUPP → LINEITEM** |
| ex23 | Rolling 3-Month Order Momentum by Segment             | Which customer segments show sustained order momentum over time, based on a rolling 3-month order count?                                   | `DATE_TRUNC`, `JOIN`, `GROUP BY`, window `SUM() OVER ROWS BETWEEN`            |     7      | **CUSTOMER**, **ORDERS**                     |
| ex24 | Strategic Customers per Segment — Deep Dive (1995)    | Who were the top 1% of customers by revenue per segment in 1995, and how do their order patterns and discount behaviour compare?           | `JOIN`, `GROUP BY`, `PERCENTILE_CONT`, `AVG()`, `COUNT()`, `CTE`              |     8      | **CUSTOMER**, **ORDERS**, **LINEITEM**       |
| ex25 | Supplier Discount Outliers — Elite Customer Focus     | Which suppliers gave unusually high or low discounts to the top 1% of customers per segment during 1995, based on statistical thresholds?   | `JOIN`, `PERCENTILE_CONT`, `AVG()`, `STDDEV()`, `CASE`, `CTE`, aggregation    |     9      | **LINEITEM**, **ORDERS**, **CUSTOMER**, **SUPPLIER** |
| ex26 | Segment Summary Dashboard Payload                     | How can we construct a JSON-style summary per segment, including total revenue, average discount, top customer and supplier counts?        | `OBJECT_CONSTRUCT`, `JOIN`, `PERCENTILE_CONT`, CTE chaining, modular metrics  |     8      | **CUSTOMER**, **ORDERS**, **LINEITEM**, **SUPPLIER** |
